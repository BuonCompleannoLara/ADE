...
async def goto_booking_portal(context: BrowserContext, page: Page) -> Page:
    """Porta il browser sulla schermata di login del portale, provando piu'
    strade in ordine di robustezza."""

    # Strada 1: URL diretto. Un browser vero esegue il JS della SPA, quindi
    # di norma funziona (a differenza di un semplice fetch senza JavaScript).
    try:
        await page.goto(BOOKING_URL, wait_until="domcontentloaded")
        await page.wait_for_timeout(2500)
        await dismiss_cookie_banner(page)
        if await wait_for_any_text(page, LOGIN_MARKERS, 12000):
            log("Portale raggiunto con URL diretto.")
            return page
        log("URL diretto non ha mostrato il login, passo dalla pagina informativa.")
    except Exception as exc:
        log(f"URL diretto fallito ({type(exc).__name__}), passo dalla pagina informativa.")

    # Strada 2: pagina informativa, leggo l'href e ci navigo.
    await page.goto(INFO_PAGE_URL, wait_until="domcontentloaded")
    await dismiss_cookie_banner(page)

    href = await find_booking_href(page)
    if href:
        log(f"Trovato link al portale nella pagina informativa: {href[:70]}...")
        await page.goto(href, wait_until="domcontentloaded")
        await page.wait_for_timeout(2500)
        await dismiss_cookie_banner(page)
        if await wait_for_any_text(page, LOGIN_MARKERS, 15000):
            return page

    # Strada 3: click vero e proprio, come ultima risorsa.
    log("Provo a cliccare l'elemento 'Appuntamento'.")
    ...
    if await click_anything(page, ["Appuntamento"]):
        ...
        await dismiss_cookie_banner(booking_page)
        if await wait_for_any_text(booking_page, LOGIN_MARKERS, 15000):
            return booking_page
        return booking_page

    raise RuntimeError(
        "Non sono riuscito ad aprire il portale prenotazioni "
        "(ne' URL diretto, ne' link nella pagina informativa, ne' click)."
    )

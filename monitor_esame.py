...
async def dismiss_cookie_banner(page: Page) -> None:
    """Se compare il banner cookie ('Navigando su questo sito accetti...'), lo
    chiude cliccando Accetta. Non fa nulla (e non blocca) se non compare."""
    for label in ["Accetta", "Accetto", "Accetta tutti", "OK", "Accept"]:
        btn = page.get_by_role("button", name=label, exact=False)
        try:
            if await btn.count() > 0:
                await btn.first.click(timeout=3000)
                await page.wait_for_timeout(500)
                log(f"Banner cookie chiuso (bottone '{label}').")
                return
        except Exception:
            continue
...
async def save_debug(page: Page) -> None:
    """Raccoglie informazioni testuali sulla pagina al momento dell'errore.
    Pensato per essere leggibile direttamente nel terminale, senza dover
    scaricare screenshot da un VPS senza schermo."""
    try:
        ...
        if idx == -1:
            log("La parola 'Appuntamento' non compare per niente nell'HTML della pagina.")
        else:
            snippet = html[max(0, idx - 100): idx + 100].replace("\n", " ")
            log(f"'Appuntamento' compare nell'HTML, contesto:\n  ...{snippet}...")

        debug_path = Path(__file__).parent / "debug_page.html"
        debug_path.write_text(html, encoding="utf-8")
        log(f"HTML completo salvato in {debug_path.name} "
            f"(es. grep -io 'appuntamento[^<]*' {debug_path.name})")
